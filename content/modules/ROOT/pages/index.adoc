...
* VM access: `virtctl console rhel-webserver -n default` (login: cloud-user, password: Enter) or `virtctl ssh cloud-user@vmi/rhel-webserver -n default` (if bastion key injected). Wait 5–10 minutes after the VM is Running for cloud-init to complete.
