...
* VM access: Console `virtctl console rhel-webserver -n default` (login: user, password: redhat). SSH `virtctl ssh -i ~/.ssh/id_ed25519 user@vmi/rhel-webserver -n default` (requires bastion key in cloud-init). Wait 5–10 minutes after the VM is Running for cloud-init to complete.
...
[source,sh,subs="attributes"]
----
ssh {openshift-base--bastion_ssh_user_name}@{openshift-base--bastion_public_hostname}
----
...
----
{openshift-base--bastion_ssh_password}
----
